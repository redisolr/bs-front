        ...
        <div style="">
            <div class="good-img">
                <a @click="openProduct(msg.id)">
                    <img :src="msg.goodsImage" :alt="msg.goodsName">
                </a>
            </div>
            <h6 class="good-title" v-html="msg.goodsName">{{msg.goodsName}}</h6>
            <h3 class="sub-title ellipsis">{{msg.goodsName}}</h3>
            <div class="good-price pr">
                <div class="ds pa">
                    ...
                    <y-button text="加入购物车"
                              style="margin: 0 5px"
                              @btnClick="addCart(msg.id,msg.goodsPrice,msg.goodsName,msg.goodsImage)"
                              classStyle="main-btn"
                    ></y-button>
                </div>
		...
		addCart(id, price, name, img) {
			if (!this.showMoveImg) {     // 动画是否在运动
				if (this.login) { // 登录了 直接存在用户名下
					addCart({goodsId: id, purchaseNumber: 1}).then(res => {
						// 并不重新请求数据
						this.ADD_CART({goodsId: id, goodsPrice: price, goodsName: name, goodsImage: img})
					})
				} else { // 未登录 vuex
					this.ADD_CART({goodsId: id, goodsPrice: price, goodsName: name, goodsImage: img})
				}
