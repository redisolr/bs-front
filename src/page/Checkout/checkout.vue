                ...
                <div slot="content">
                    <ul class="address-item-list clearfix">
                        <li v-for="(item,i) in addList"
                            ...
                            @click="chooseAddress(item.id, item.contactName, item.contactPhone, item.addressDetail)">
           <span v-if="id === item.id" class="pa">
             ...
             <path
                     ...
                     fill="#6A8FE5" p-id="1025">
                            ...
                            <p>收货人: {{item.contactName}} {{item.isDefault ? '(默认地址)' : ''}}</p>
                            <p class="street-name ellipsis">收货地址: {{item.addressDetail}}</p>
                            <p>手机号码: {{item.contactPhone}}</p>
                    ...
                    <div class="box-inner ui-cart">
                        <div>
                            ...
                            <div class="cart-table" v-for="(item,i) in cartList" :key="i" v-if="item.checked">
                                <div class="cart-group divide pr" :data-id="item.id">
                                        ...
                                        <div class="cart-items clearfix">
                                            ...
                                            <div class="items-thumb fl">
                                                <img :alt="item.productName"
                                                     :src="item.goodsImage">
                                                <a @click="goodsDetails(item.id)" :title="item.productName"
                                                   target="_blank"></a>
                                            </div>
                                            <!--信息-->
                                            ...
                                            <div>
                                                <!--总价格-->
                                                <div class="subtotal" style="font-size: 14px">¥ {{item.goodsPrice *
                                                    item.purchaseNumber | priceFormat}}
                                                </div>
                                                <!--数量-->
                                                <div class="item-cols-num">
                                                    <span v-text="item.purchaseNumber"></span>
                                                </div>
                                                <!--价格-->
            ...
            <y-popup :open="popupOpen" @close='popupOpen=false' :title="popupTitle">
                <div slot="content" class="md" :data-id="msg.id">
                    <div>
                        <input type="text" placeholder="收货人姓名" v-model="msg.contactName">
                    </div>
                    <div>
                        <input type="number" placeholder="手机号码" v-model="msg.contactPhone">
                    </div>
                    <div>
                        <input type="text" placeholder="收货地址" v-model="msg.addressDetail">
                    </div>
                    <div>
                    ...
                    <y-button text='保存'
                              ...
                              @btnClick="save({userId:userId,id:msg.id,contactName:msg.contactName,contactPhone:msg.contactPhone,addressDetail:msg.addressDetail,isDefault:msg.isDefault})">
                    </y-button>
                </div>
            </y-popup>
        </div>
...
import {addressList, addressUpdate, addressAdd, addressDel,getCartList} from '../../api/user'
...
export default {
	...
	computed: {
		btnHighlight() {
			let msg = this.msg
			return msg.contactName && msg.contactPhone && msg.addressDetail
		},
		// 选中的总价格
		checkPrice() {
			let totalPrice = 0
			this.cartList && this.cartList.forEach(item => {
				if (item.checked) {
					totalPrice += (item.purchaseNumber * item.goodsPrice)
				}
			})
	...
	methods: {
		...
		_addressList() {
			addressList().then(res => {
				...
				if (data.length) {
					...
					this.contactPhone = data[0].contactPhone
		...
		_submitOrder() {
			...
			let params = {
				receiverPhone: this.contactPhone,
				receiverName: this.contactName,
				receiverAddress: this.addressDetail,
				orderGoodsList: array,
				payment: this.orderTotal
			}
		...
		// 选择地址
		chooseAddress(id, contactName, contactPhone, addressDetail) {
			this.id = id
		...
		update(item) {
			...
			if (item) {
				...
				this.msg.contactName = item.contactName
				this.msg.contactPhone = item.contactPhone
				this.msg.addressDetail = item.addressDetail
				this.msg.isDefault = item.isDefault
				this.msg.id = item.id
			} else {
				this.popupTitle = '新增收货地址'
				this.msg.contactName = ''
		...
		del(id) {
			...
			this._addressDel(params)
		},
		_productDet(id) {
			productDet({params: {id: id}}).then(res => {
				let item = res.result
				item.checked = true
				let images = item.goodsImage.split(',')
				item.goodsImage = images[0]
				item.purchaseNumber = this.num
				item.goodsDesc = ''
