            ...
            <div slot="content">
                <div v-loading="loading" element-loading-text="加载中..." style="min-height: 10vw;"
                    ...
                    <div class="orderStatus-close" v-if="orderStatus === 6">
                        <el-steps :space="780" :active="2">
                            <el-step title="下单" v-bind:description="createTime"></el-step>
                            <el-step title="交易失败" v-bind:description="closeTime"></el-step>
                        </el-steps>
                    </div>
                    ...
                    <div class="goods-table">
                        <div class="cart-items" v-for="(item,i) in orderList" :key="i">
                            <a @click="goodsDetails(item.goodsId)" class="img-box"><img :src="item.goodsImage"
                                                                                        alt=""></a>
                            <div class="name-cell ellipsis">
                                <a @click="goodsDetails(item.goodsId)" title=""
                                   target="_blank">{{item.goodsName}}</a>
                            </div>
                            <div class="n-b">
                                <div class="price">¥ {{Number(item.goodsPrice).toFixed(2)}}</div>
                                <div class="goods-num">{{item.purchaseNumber}}</div>
                                <div class="subtotal"> ¥ {{Number(item.totalFee).toFixed(2)}}
                                </div>
                            </div>
                        </div>
                    </div>
...
export default {
	...
	methods: {
		...
		_getOrderDet() {
			getOrderDet(this.orderId).then(res => {
				...
				this.orderList = res.result.orderGoodsList
				this.orderTotal = res.result.payment
				this.userName = res.result.orderShipping.receiverName
				this.tel = res.result.orderShipping.receiverPhone
				this.streetName = res.result.orderShipping.receiverAddress
				this.createTime = res.result.gmtCreate
				this.closeTime = res.result.closeTime
				this.payTime = res.result.paymentTime
				this.finishTime = res.result.finishTime
				if (this.orderStatus === 1) {
					let time_Date = new Date(res.result.gmtCreate)
					this.countTime = String(time_Date.getTime() + 24 * 60 * 60 * 1000);
				}
				this.loading = false
			})
		},
