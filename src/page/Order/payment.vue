            ...
            <div slot="content">
                ...
                <div class="pay-info">
                    ...
                    <el-select class="money-select" v-model="moneySelect" placeholder="请选择支付金额" @change="changeSelect">
                        ...
                        <el-option label="￥6.66 感谢支持" value="6.66"></el-option>
            ...
            <div class="cart-items" v-for="(item,i) in cartList" :key="i">
                <div class="name">
                    <div class="name-cell ellipsis">
                        <a @click="goodsDetails(item.goodsId)" title="" target="_blank">{{item.goodsName}}</a>
                    </div>
                </div>
                <div class="n-b">
                    ...
                    <div class="subtotal">
                        <div class="subtotal-cell"> ¥ {{Number(item.totalFee).toFixed(2)}}<br></div>
                    </div>
                </div>
            </div>
        </div>
...
import {getOrderDet} from '../../api/order'
	...
	methods: {
		...
		_getOrderDet(orderId) {
			getOrderDet(this.orderId).then(res => {
				this.cartList = res.result.orderGoodsList
