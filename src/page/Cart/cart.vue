                                    ...
                                    <div class="cart-top-items">
                                        <div class="cart-items clearfix">
                                            ...
                                            <span class="blue-checkbox-new "
                                                  ...
                                                  @click="editCart(item)"></span>
                                            </div>
                                            <!--图片-->
                                            <div class="items-thumb fl">
                                                <img :alt="item.goodsName"
                                                     :src="item.goodsImage">
                                                <a @click="goodsDetails(item.goodsId)" :title="item.goodsName"
                                                   target="_blank"></a>
                                            </div>
                                            <!--信息-->
                                            ...
                                            <div>
                                                <!--总价格-->
                                                <div class="subtotal" style="font-size: 14px">¥ {{item.goodsPrice * item.purchaseNumber | priceFormat}}
                                                </div>
                                                <!--数量-->
                                                <buy-num :num="item.purchaseNumber"
                                                         :id="item.goodsId"
...
<script>
import {getCartList, cartEdit, editCheckAll, cartDel, delCartChecked} from '../../api/user'
...
export default {
	...
	methods: {
		...
		// 修改购物车
		_cartEdit(goodsId, purchaseNumber, checked) {
			cartEdit(
				{
					goodsId,
					purchaseNumber,
		...
		editCart(item) {
			...
			this._cartEdit(goodsId, purchaseNumber, checked)
		},
		EditNum(purchaseNumber, goodsId, checked) { // 数量
			this._cartEdit(goodsId, purchaseNumber, checked)
		},
		// 删除整条购物车
		cartdel(goodsId) {
