                ...
                <div class="w-box">
                    <div class="nav-logo">
                        <h1 @click="changePage(-1)">
                            <router-link to="/" title="图书商城官网">图书商城</router-link>
                        </h1>
                    </div>
                    <div class="right-box">
                        <div class="nav-list">
                                    ...
                                    <div class="nav-user-list">
                                        <ul>
                                            ...
                                            <li class="nav-user-avatar">
                                                <div>
                          <span class="avatar" :style="{backgroundImage:'url('+userInfo.info.userHead+')'}">
                          </span>
                                                </div>
                                                    ...
                                                    <li class="clearfix" v-for="(item,i) in cartList" :key="i">
                                                        <div class="cart-item">
                                                            <div class="cart-item-inner">
                                                                <a @click="openProduct(item.goodsId)">
                                                                    ...
                                                                    <div class="item-desc">
                                                                        <div class="cart-cell"><h4>
                                                                            ...
                                                                            <h6><span class="price-icon">¥</span><span
                                                                                    ...
                                                                                    class="item-num">x {{item.purchaseNumber}}</span>
                                                                            </h6></div>
                                                                    </div>
                                                                </a>
                                                                <div class="del-btn del"
                                                                     @click="delGoods(item.goodsId)">删除
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </li>
...
import {getCartList, cartDel} from '../api/user'
			...
			getSearch(params).then(res => {
				if (res.status === 0) {
					...
					if (res.result.data.length <= 5) {
						maxSize = res.result.data.length
					}
					for (var i = 0; i < maxSize; i++) {
						var obj = {}
						obj.value = res.result.data[i].goodsName
						array.push(obj)
					}
					if (array.length !== 0) {
...
<style lang="scss" rel="stylesheet/scss" scoped>
    ...
    .header-box {
        background: $head-bgc;
        background-image: -webkit-linear-gradient(#3e58c9, #3e58c9);
        background-image: linear-gradient(#010101, #555a5c);
        width: 100%;
