...
export default {
	...
	// 加入购物车
	[ADD_CART](state, {goodsId, goodsPrice, goodsName, goodsImage, purchaseNumber = 1}) {
		...
		if (!cart.length || falg) {
			goods.purchaseNumber = purchaseNumber
			goods.checked = true
			cart.push(goods)
		}
		state.cartList = cart
	...
	// 修改购物车
	[EDIT_CART](state, {goodsId, purchaseNumber, checked}) {
		let cart = state.cartList
		if (purchaseNumber) {
			cart.forEach((item, i) => {
				if (item.goodsId === goodsId) {
